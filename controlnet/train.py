...
#  - Convert 3-channel RGB input into 97-channel one-hot embeddings
...
def make_train_dataset(args, tokenizer, accelerator):
    with accelerator.main_process_first():    
        ...
        if args.dataset_type == 'ADE20K':
            train_dataset = ADE20KDataset(args, tokenizer) 
...
def main(args):
    ...
    if args.partial_freeze_expid == 1:
        ...
        for param in controlnet.time_proj.parameters():
            param.requires_grad = False
        for param in controlnet.time_embedding.parameters():
            param.requires_grad = False
        if controlnet.class_embedding:
            for param in controlnet.class_embedding.parameters():
                param.requires_grad = False
    ...
    count_trainable_params = sum(p.numel() for p in controlnet.parameters() if p.requires_grad)
    logger.info(f"Number of trainable parameters: {count_trainable_params}")
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ckpt_save_path = os.path.join(args.output_dir, f"checkpoint-{global_step}")
